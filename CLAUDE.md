...
- `GET  /api/ltl/dock/presence?tractor={t}&trailer={r}&driverId={d}` — Phase 2.5 Dock mode / issue #166: Yard-presence projection for the combine Review-step chip. Honest grey "unavailable" when the Yard integration is off or unreachable; presence is never fabricated into a pass. Read-only; Alvys stays authoritative.
- `GET  /api/ltl/dock/opportunities?max={n}` — Phase 2.5 Dock mode / issue #166: yard-originated LTL consolidation opportunities (from `LtlDraftCreated` webhooks), newest first. Inbound suggestions only — the dock acts on them inside its own Alvys-backed combine flow. Read-only.
- `GET  /api/ltl/dispatch/recommendations?loadId={id}&originCity=&originState=&destinationCity=&destinationState=&top={n}` — Dispatch Assist: ranked, explainable driver+truck+trailer candidates for a load (by `loadId`) or an ad-hoc lane. Assembled from Alvys drivers/trucks/trailers/dispatch-preferences; every row carries the reasons behind its score. Read-only; nothing written to Alvys. 404 only when a supplied `loadId` is unresolvable.
- `POST /api/ltl/dispatch/assemble` — Dispatch Assist: records the dispatcher's chosen driver+truck+trailer **app-side only** (`AlvysWriteback = NotPerformed`) and fires the notify step (driver + dispatcher, resolved from read-only Alvys contacts). Comms are flag-gated (`Ltl:Comms:Enabled`, default OFF) and rerouted to `Ltl:Comms:OverrideRecipient` (default `joshua.davis@valuetruck.com`) while under override. The Alvys write seam is a no-op hook for a separate workstream.
- `GET  /api/ltl/dispatch/assemblies?loadId={id}&max={n}` — Dispatch Assist: recent app-side assembly records (newest first), optionally filtered to one load. Read-only.
- `GET  /api/ltl/invoices?parentLoadId={id}&status={Draft|Final}&max={n}` — Wave 2 Invoice Studio: assembled-invoice list (newest-updated first), filterable by parent load and draft/final status. Read-only.
- `POST /api/ltl/invoices` — Invoice Studio: assemble a customer invoice from a consolidation (parent + sibling loads, editable charges); computes totals + combined driver-RPM and tracks per-load BOL presence. Persists app-side only (`AlvysWriteback = NotPerformed`).
- `GET  /api/ltl/invoices/{id}` — Invoice Studio: one assembled invoice (loads, charges, totals, edit history). Read-only.
- `PUT  /api/ltl/invoices/{id}` — Invoice Studio: edit a **draft** invoice and append edit history; 409 when the invoice is Final (locked). App-side only.
- `POST /api/ltl/invoices/{id}/finalize` — Invoice Studio: lock a draft to Final (idempotent). App-side only.
- `GET  /api/ltl/invoices/{id}/pdf` — Invoice Studio: professional invoice PDF for download/attachment. Read-only.
- `GET  /api/ltl/invoices/{id}/alvys-preview?parentLoadEtag={etag}` — Invoice Studio: gated Alvys write payload previews (load-update OrderNumber, BOL/PDF upload, customer-payment post). Every payload resolves to AuditOnly/Blocked and is never executed until the AllowProduction gate is enabled. Read-only.
- `POST /api/yard/webhooks/receiver` — issue #166: inbound Yard webhook receiver (`TruckArrived` / `LoadReleased` / `LtlDraftCreated`). HMAC-signed (`X-Yard-Signature: t=…,v1=…`), fail-closed (503 without a secret), gated behind `Yard:Webhooks:Enabled` (404 when off). Never writes to Alvys.
- `GET  /api/yard/webhooks/events?max={n}` — issue #166: read-only admin listing of recent Yard webhook deliveries + honest receiver configuration (behind `AllowedEmailDomain`). Backs the `/admin/yard/webhooks` panel.
