...
# matplotlib is optional: it is only used by the plot()/fill() helpers below and by
# the demo CLI. The headless FastAPI server never plots, so we guard the import to
# avoid pulling a GUI/plotting stack into the container. Calling plot()/fill()
# without matplotlib installed raises a clear error.
try:  # pragma: no cover - import guard
	from matplotlib import pyplot
except ImportError:  # pragma: no cover - exercised only in headless builds
	pyplot = None
...
def _require_pyplot():
	if pyplot is None:
		raise RuntimeError(
			'matplotlib is not installed; plotting is unavailable in this build'
		)
	return pyplot
...
class ShippingObject(LogisticsObject):
	...
	def plot(self,*args,**kwargs):
		...
		_require_pyplot().plot(x_list,y_list,**kwargs)
	...
	def fill(self,*args,**kwargs):
		...
		_require_pyplot().fill(x_list,y_list,**kwargs)
