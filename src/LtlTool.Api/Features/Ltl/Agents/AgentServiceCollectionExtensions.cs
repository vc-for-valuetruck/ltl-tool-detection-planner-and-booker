...
/// digest, billing-ready sweeper) and their durable heartbeat store. Options bind from
/// <c>Ltl:Agents</c>; every agent is OFF by default. The hosted services are always registered but
/// self-gate on their <c>Ltl:Agents:*:Enabled</c> flag — a disabled agent records a single honest
/// 'off' heartbeat and sweeps nothing, which keeps the heartbeat surface truthful (an off agent
/// shows as 'off', not absent) and the no-op path unit-testable.
///
...
public static class AgentServiceCollectionExtensions
{
        ...
        this IServiceCollection services, IConfiguration configuration)
    {
        ...
        // Four hosted sweepers. Always registered, self-gated on their Enabled flag.
